...
public class PolicyShow : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            // If no policies are available, then the game just started. Try to load data.
            if(policyCount == 0 ){
                Policy firstPolicy,secondPolicy, thirdPolicy;
                
                if(PlayerPrefs.HasKey("firstPolicy")){
                    int firstID = PlayerPrefs.GetInt("firstPolicy");
                    firstPolicy = policyScript.policyDictionary[firstID];
                }
                if(PlayerPrefs.HasKey("secondPolicy")){
                    int secondID = PlayerPrefs.GetInt("secondPolicy");
                    secondPolicy = policyScript.policyDictionary[secondID];
                }
                if(PlayerPrefs.HasKey("thirdPolicy")){
                    int thirdID = PlayerPrefs.GetInt("thirdPolicy");
                    thirdPolicy = policyScript.policyDictionary[thirdID];
                }
